...
class WorldObject:
    ...
    def uvCoordinates(self, vec):
        """ Return the uv coordinates which can be used to texture the object. """
        raise Exception("Function undefined: getTexturePos()")
    ...
    def surfaceColor(self, vec):
        ...
        if (isinstance(self.color, tuple)):
            return self.color
        elif (isinstance(self.color, Texture)):
            texture = self.color
            uvCoodinates = self.uvCoordinates(vec, texture.resolution)
            return texture.getColor(uvCoodinates)
    ...
    def getColorFromRay(self, scene, vec, viewPoint, recur=0):
        ...
        for light in scene.lights:
            colorFromLight = self._getColorFromLight(scene, vec, light, viewPoint, surfaceColor)
    ...
    def _getColorAmbient(self, scene, vec, surfaceColor):
